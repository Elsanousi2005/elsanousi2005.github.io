...
![The image is a very wide, short horizontal strip that appears uniformly white with no discernible objects, text, or details.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_001.png)
...
![The image shows a mostly uniform, very light (near‑white) horizontal strip with subtle gray edges and no clearly discernible objects, text, or features.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_004.png)

## 2. System architecture and responsibilities
...
![- A block-diagram of the Ethernet datapath between a Zynq-7000 Processing System (PS) and the ZC702 board PHY. - Left: "Zynq-7000 Processing System (PS)" block. - Center (inside PS): "GEM0 Ethernet MAC" performing framing, buffering and driver functions. - Right (on ZC702 board): "GigE PHY" (analog + line interface) connected onward to "RJ45 + Magnetics". - Two labeled signal links between MAC and PHY: - RGMII data/clock (TX/RX) — bidirectional data and clock lines. - MDIO/MDC (management) — MDIO management interface.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_008.png)

*Figure 3: Conceptual MAC to PHY diagram showing RGMII data signals and MDIO management channel*

![A wide, mostly blank white horizontal rectangle with no discernible objects, text, or features (appears like an empty or overexposed image).](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_009.png)

## 4. Digital pin routing modes in Zynq: MIO and EMIO
...
![Diagram showing how a hardened PS peripheral is routed to external pins or PL logic via EMIO and PL routing: - Left box: Processing System (PS) - "Hardened PS Peripheral" → arrow to "EMIO Interface" - EMIO Interface output continues into the right box: Programmable Logic (PL) - Inside PL: "PL Routing / Logic" receives the EMIO signal - A separate input labeled "Requires PL configuration + clocks" feeds into the PL routing/logic - PL Routing / Logic → arrow to "External Pins / Internal PL Logic" (rightmost block) Overall: the PS peripheral uses the EMIO path into the PL, where configured PL routing/logic (and clocks/configuration) drive external pins or other PL logic.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_011.png)
...
![- The image appears almost entirely blank/white with no discernible objects, text, or features — looks overexposed or intentionally empty.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_012.png)

## 5. How the PS communicates with the PL: AXI GP, SmartConnect, and AXI-Lite
...
![The image appears as an extremely wide, short white strip with no discernible content or features — essentially a blank or overexposed canvas.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_015.png)

## 6. The PL datapath: AXI GPIO out to inverter to AXI GPIO in
...
![The image appears to be a long, narrow horizontal white/blank area with no discernible objects, text, or notable features.](./Ethernet_Bring-Up_Post1_Elsanousi2005%20_images/image_017.png)
